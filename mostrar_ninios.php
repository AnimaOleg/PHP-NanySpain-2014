<?php
	...
	if(isset($_POST['busqueda']) && isset($_POST['busquedaRadio']) != null){
		...
		switch($_POST['busquedaRadio']){
			...
			default:
				$queRadio="ninguno";
				$consulta1 = "SELECT * FROM usuarios where tipoUsuario='ninos'";
				break;
		}
		$resultado = mysql_query($consulta1, $conexion) or die(mysql_error());
		
		while($fila = mysql_fetch_assoc($resultado)){
			$consulta2= "select * from usuarios where dni='".$fila['dni_usuario']."' and tipoUsuario='ninios'";
		...
		$consulta = "SELECT * FROM usuarios where tipoUsuario='ninios'";
		...
		while($fila = mysql_fetch_assoc($resultado)){
			...
			if(isset($_SESSION['logged'])){
				...
				$fila2 = mysql_fetch_assoc($resultado2);
				if($fila2['tipoUsuario'] != "ninios"){
					$div .= "<form action='funciones.php' method='post'><input type='submit' name='conectar' value='Conectar' class='botonLogin'></input>
					<input type='text' name='usuarioConectado' value='".$fila['usuario']."' hidden/></form>";
				}
			}		
			$div .= "</div></a>";
